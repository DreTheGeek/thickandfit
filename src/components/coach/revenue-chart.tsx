    ...
    <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full" preserveAspectRatio="xMidYMid meet" role="img">
      <defs>
        <linearGradient id="gGross" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="var(--c-ink)" stopOpacity={0.16} />
      ...
      {ticks.map((tk, i) => (
        <g key={i}>
          ...
          <text x={padL - 8} y={y(tk) + 3} textAnchor="end" fontSize="10" fill="var(--c-faint)">
            {tick(tk)}
          </text>
        </g>
      ))}
      ...
      <path d={line('gross')} fill="none" stroke="var(--c-ink)" strokeWidth={2} />
      <path d={line('coach')} fill="none" stroke="#5EBE62" strokeWidth={2} />

      {data.map((d, i) => (
        <circle key={i} cx={x(i)} cy={y(d.gross)} r={2.5} fill="var(--c-ink)">
