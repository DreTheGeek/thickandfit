...
import { aiConfigured, embed } from '@/lib/ai/client';
...
// Embed one string via the shared client. Returns the vector, or null when the key is absent or the
// call fails (callers degrade: no embedding stored / no RAG injected). Never throws. Dims validation
// stays HERE: the coach_memory pgvector column is locked to EMBED_DIMS.
export async function embedText(input: string): Promise<number[] | null> {
  const text = input.trim();
  if (!text) return null;
  const vec = await embed(EMBED_MODEL, text);
