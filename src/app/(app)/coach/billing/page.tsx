...
export default async function CoachBillingPage(): Promise<ReactElement> {
  ...
  return (
    <div className="px-[22px] py-6">
      <PageTitle className="mb-5">Billing & Renewals</PageTitle>

      <div className="mb-6 grid grid-cols-3 gap-3">
        <div className="rounded-2xl bg-ink p-4 text-bg">
          <div className="text-[11px] uppercase tracking-[2px] opacity-70">MRR</div>
          <div className="tf-display mt-1 text-[24px]">{money(data.mrrCents, 'usd')}</div>
        </div>
        <div className="rounded-2xl border border-line p-4">
          <div className="text-[11px] uppercase tracking-[2px] text-faint">Renew in 30d</div>
          <div className="tf-display mt-1 text-[24px] text-ink">{data.renewSoon}</div>
        </div>
        <div className="rounded-2xl border border-line p-4">
          <div className="text-[11px] uppercase tracking-[2px] text-faint">Failed / overdue</div>
      ...
      <div className="overflow-hidden rounded-2xl border border-line">
        <div className="grid grid-cols-[1fr_5rem_6rem_6rem] gap-3 border-b border-line bg-surface px-4 py-2 text-[11px] font-semibold uppercase tracking-[1px] text-faint">
          ...
          <span className="text-right">Next billing</span>
          <span className="text-right">Status</span>
        </div>
        {data.rows.length === 0 ? (
          <div className="px-4 py-10 text-center text-[13px] text-faint">No subscriptions yet.</div>
        ) : (
          data.rows.map((r) => (
            <div
              key={r.contactId}
            ...
            >
              ...
              >
                {r.flag === 'failed' ? 'Failed' : r.flag === 'soon' ? 'Scheduled' : (r.status ?? '-')}
              </span>
            </div>
          ))
        )}
