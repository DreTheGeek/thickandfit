...
(async () => {
  ...
  await p.screenshot({ path: path.join(OUT, 'toggle-topbar.png') });
  console.log('toggle-topbar');
  await p.evaluate(() => localStorage.setItem('theme', 'dark'));
  await p.goto(`${BASE}/coach/tool/recipes`, { waitUntil: 'networkidle2' });
  await new Promise((r) => setTimeout(r, 2200));
  await p.screenshot({ path: path.join(OUT, 'recipes-dark.png') });
  console.log('recipes-dark');
  await b.close();
  console.log('done');
